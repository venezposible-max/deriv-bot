...
console.log(`\n🔍 BACKTEST DETALLADO: ORO - JUEVES 19 FEB`);
console.log(`==========================================================`);
...
ws.on('open', () => {
    // Pedir H1 (necesitamos los días previos para la SMA de 1H)
    ws.send(JSON.stringify({
        ticks_history: SYMBOL, end: 'latest', count: 1000, granularity: 3600, style: 'candles'
    }));
    // Pedir M1
    ws.send(JSON.stringify({
...
function runThursdayBacktest() {
    ...
    for (let i = 40; i < m1Candles.length; i++) {
        ...
        if (m1S20[i] > m1S40[i] && h1TrendUp) {
            ...
            if (c.low <= m1S40[i] * 1.0002) {
                ...
                // Buscar ruptura en las siguientes velas (max 15 min)
    ...
    function simulateTrade(startIdx) {
        ...
        return -SL; // Time out o fin de datos
    }

    console.log(`\n--------------------------------------------------`);
    console.log(`🏆 RESUMEN JUEVES 19 FEB:`);
