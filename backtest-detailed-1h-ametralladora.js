...
// CONFIGURACIÓN ACTUAL "AMETRALLADORA" (La que tienes en el server)
const CONFIG = {
    stake: 20,
    takeProfit: 3.00,
    ...
    distLimit: 0.15,
    trailStart: 0.50,
    trailDist: 0.55      // Latencia en el cierre del trailing
};

ws.on('open', () => {
    console.log(`📥 Descargando DATA TICKS para REPORTE DETALLADO (Última Hora)...`);
    fetchTicks();
});
...
ws.on('message', (data) => {
    ...
    if (msg.msg_type === 'history') {
        ...
        if (allTicks.length < TICKS_TO_FETCH && chunk.length > 0) {
            ...
            console.log(`\n✅ DATA CARGADA. Generando log detallado de la última hora...`);
            runSimulation();
            ws.close();
        }
    }
...
function runSimulation() {
    ...
    // Tomamos la ventana de tiempo para 60 minutos (aprox 3600 ticks) + margen
    const simulationTicks = allTicks.slice(-4000);

    console.log(`\n====================================================`);
    console.log(`📝 LOG DETALLADO: ÚLTIMA HORA (MODO AMETRALLADORA)`);
    console.log(`====================================================`);
        ...
        if (!inTrade) {
            ...
            if (sma50 && sma200 && rsi) {
                ...
                if (distPct < CONFIG.distLimit) {
                    let dir = null;
                    if (allUp && quote > sma200 && rsi > CONFIG.rsiLow) dir = 'UP';
                    else if (allDown && quote < sma200 && rsi < CONFIG.rsiHigh) dir = 'DOWN';

                    if (dir) {
                        inTrade = true; tradeType = dir; entryPrice = quote; maxProfit = 0; lastSl = -1.50; trades++;
                    }
                }
            }
        } else {
            ...
            // Lógica de Trailing de tu servidor
            if (maxProfit >= 0.50) {
                const currentStep = Math.floor(maxProfit / 0.50) * 0.50;
                const newFloor = currentStep - 0.55; // Simulando latencia de cierre
