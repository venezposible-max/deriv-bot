...
const CONFIG = {
    ...
    rsiLow: 25,
    rsiHigh: 75,
    momentum: 5,         // Nueva confirmación sólida
    distLimit: 0.08,     // Nueva precisión extrema
    trailStart: 0.50,
    trailDist: 0.55
};

ws.on('open', () => {
    console.log(`📥 Descargando DATA TICKS para REPORTE 8H (Sniper 0.08 + Momentum 5)...`);
    fetchTicks();
});
...
ws.on('message', (data) => {
    ...
    if (msg.msg_type === 'history') {
        ...
        if (allTicks.length < TOTAL_TICKS_NEEDED && chunk.length > 0) {
            ...
            console.log(`\n✅ DATA OK: ${allTicks.length} ticks. Analizando 8 Horas...`);
            runFinal8hSim();
            ws.close();
        }
    }
...
function runFinal8hSim() {
    ...
    console.log("=========================================");
    console.log("🏆 REPORTE 8 HORAS SINCERO: SNIPER 0.08");
    console.log("=========================================");
    console.log(`Filtro Distancia: 0.08% (Máxima Exigencia)`);
    console.log(`Momentum Requerido: 5 ticks seguidos`);
    console.log(`-----------------------------------------`);
