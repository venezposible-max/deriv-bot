...
ws.on('open', () => {
    console.log(`📥 Descargando DATA TICKS para 12 HORAS (~40k ticks)...`);
    fetchTicks();
});
...
ws.on('message', (data) => {
    ...
    if (msg.msg_type === 'history') {
        ...
        // El historial llega con el más reciente al final
        allTicks = [...chunk, ...allTicks];

        if (allTicks.length < TOTAL_TICKS_NEEDED && chunk.length > 0) {
            process.stdout.write('.');
            fetchTicks(times[0]); // Pedimos antes del primer tick recibido
        } else {
...
function runSimulation() {
    ...
    console.log(`\n📊 SIMULANDO 12 HORAS (TICK-BY-TICK)...`);
    ...
    console.log("=========================================");
    console.log("🏆 RESULTADO FINAL 12 HORAS (TICKS)");
    console.log("=========================================");
