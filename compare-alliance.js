...
ws.on('open', () => {
    console.log(`📥 Probando FILTRO DE ALIANZA (SMA 50 vs SMA 200)...`);
    fetchTicks();
});
...
function testAlliance(useAlliance) {
    ...
    for (let i = 250; i < allTicks.length; i++) {
        if (!inTrade) {
            ...
            if (sma50 && sma200 && rsi && volOK) {
                ...
                let trigger = null;
                if (distPct < 0.12 && rsi > 25 && rsi < 75) {
                    if (allUp && allTicks[i] > sma200 && trend1m === 'UP') {
                        if (!useAlliance || sma50 > sma200) trigger = 'UP';
                    } else if (allDown && allTicks[i] < sma200 && trend1m === 'DOWN') {
                        if (!useAlliance || sma50 < sma200) trigger = 'DOWN';
                    }
                }
    ...
    console.log(`Alliance ${useAlliance}: PnL $${balance.toFixed(2)} | Trades: ${wins + losses} | WinRate: ${((wins / (wins + losses)) * 100).toFixed(1)}%`);
}
