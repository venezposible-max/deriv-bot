...
let allTicks = [];
const TOTAL_TICKS_NEEDED = 40000; // ~12 horas
...
ws.on('open', () => {
    console.log(`📥 Descargando DATA TICKS para REPORTE TOTAL SIN RESUMEN (12H)...`);
    fetchTicks();
});
...
ws.on('message', (data) => {
    ...
    if (msg.msg_type === 'history') {
        ...
        if (allTicks.length < TOTAL_TICKS_NEEDED && chunk.length > 0) {
            ...
            console.log(`\n✅ DATA CARGADA. Generando reporte completo de cada trade...`);
            runSimulation();
            ws.close();
        }
    }
...
function runSimulation() {
    ...
    let report = "Nº | TIPO | RESULTADO | PnL | BALANCE ACUM.\n";
    report += "----------------------------------------------------\n";
    ...
    for (let i = 250; i < allTicks.length; i++) {
        ...
        if (!inTrade) {
            ...
            if (prof <= -CONFIG.stopLoss) {
                ...
                pnl = lastSl; resultType = pnl > 0 ? "🛡️ TRAIL" : "🛡️ B-EVEN"; closed = true;
            }
            ...
            if (closed) {
                ...
                report += `${totalTrades.toString().padStart(3)} | ${tradeType.padEnd(4)} | ${resultType.padEnd(8)} | $${pnl.toFixed(2).padStart(6)} | $${balance.toFixed(2).padStart(8)}\n`;
            }
        }
    }

    const fs = require('fs');
    fs.writeFileSync('/Users/franklincolina/.gemini/antigravity/scratch/deriv-bot/FULL_TRADES_REPORT.txt', report);
    console.log(`\n✅ REPORTE GENERADO EN: FULL_TRADES_REPORT.txt`);
    console.log(`Total Trades procesados: ${totalTrades}`);
}
