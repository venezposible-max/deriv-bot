...
let allTicks = [];
const TOTAL_TICKS_NEEDED = 500000; // ~1 semana de ticks para R_100
...
ws.on('open', () => {
    console.log(`📥 Descargando DATA TICKS para 1 SEMANA (~500k ticks)...`);
    fetchTicks();
});
...
ws.on('message', (data) => {
    ...
    if (msg.msg_type === 'history') {
        ...
        if (allTicks.length < TOTAL_TICKS_NEEDED && chunk.length > 0) {
            // Un poco de feedback para no desesperar
            if (allTicks.length % 50000 === 0) {
                console.log(`📦 Progreso: ${allTicks.length} ticks descargados...`);
            } else {
...
function runSimulation() {
    ...
    console.log(`\n📊 SIMULANDO 1 SEMANA (TICK-BY-TICK)...`);
    ...
    console.log("=========================================");
    console.log("🏆 RESULTADO FINAL 1 SEMANA (TICKS)");
    console.log("=========================================");
