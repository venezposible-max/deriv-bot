...
ws.on('open', () => {
    console.log(`📥 Comparando Momentum 3 vs 5 para el periodo de decepción...`);
    fetchTicks();
});
...
function compare(momentumVal) {
    const CONFIG = {
        stake: 20, takeProfit: 3.00, stopLoss: 1.80, multiplier: 40,
        smaPeriod: 50, smaLongPeriod: 200, rsiPeriod: 14,
        rsiLow: 25, rsiHigh: 75, momentum: momentumVal, distLimit: 0.12
    };

    let balance = 0, wins = 0, losses = 0, inTrade = false, entryIdx = 0, entryPrice = 0, tradeType = null, maxProfit = 0, lastSl = -100;
    ...
    console.log(`Momentum ${momentumVal}: PnL $${balance.toFixed(2)} | Trades: ${wins + losses} | WinRate: ${((wins / (wins + losses)) * 100).toFixed(1)}%`);
}
