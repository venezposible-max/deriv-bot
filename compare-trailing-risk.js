...
ws.on('open', () => {
    console.log(`\n📥 COMPARATIVA CRÍTICA: TRAILING VS NO-TRAILING (24H)...`);
    fetchTicks();
});
...
function runSim(useTrailing) {
    ...
    for (let i = 250; i < allTicks.length; i++) {
        ...
        if (!inTrade) {
            ...
            if (useTrailing && maxProfit >= trailStart) {
            ...
            else if (useTrailing && lastSl > -90 && prof <= lastSl) { pnl = lastSl; closed = true; }
...
function runComparison() {
    console.log("\n=========================================");
    console.log("🕵️‍♂️ COMPARATIVA ESTRATÉGICA (24H)");
    console.log("=========================================");

    const withT = runSim(true);
    const withoutT = runSim(false);

    console.log(`🛡️ CON TRAILING (Modo Seguro):`);
    console.log(`   PnL: $${withT.balance.toFixed(2)} | WR: ${((withT.wins / withT.trades) * 100).toFixed(1)}%`);
    console.log(`   Ganadas: ${withT.wins} | Perdidas: ${withT.losses}`);

    console.log(`\n⚔️ SIN TRAILING (Modo Directo):`);
    console.log(`   PnL: $${withoutT.balance.toFixed(2)} | WR: ${((withoutT.wins / withoutT.trades) * 100).toFixed(1)}%`);
    console.log(`   Ganadas: ${withoutT.wins} | Perdidas: ${withoutT.losses}`);
