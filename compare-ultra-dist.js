...
ws.on('open', () => {
    console.log(`📥 Probando distLimit 0.05 (Ultra Precisión)...`);
    fetchTicks();
});
...
function testDist(distVal) {
    ...
    for (let i = 250; i < allTicks.length; i++) {
        if (!inTrade) {
            const lastTicks = allTicks.slice(i - 5, i); // Momentum 5
            ...
            if (sma50 && sma200 && rsi && volOK) {
                ...
                if (distPct < distVal && rsi > 25 && rsi < 75) {
    ...
    console.log(`distLimit ${distVal}: PnL $${balance.toFixed(2)} | Trades: ${wins + losses} | WinRate: ${((wins / (wins + losses)) * 100).toFixed(1)}%`);
}
