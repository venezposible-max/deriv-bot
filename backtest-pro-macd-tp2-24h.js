...
const CONFIG = {
    stake: 20,
    takeProfit: 2.00,  // TP BAJADO A $2.00
    stopLoss: 3.00,
    multiplier: 750,
    smaPeriod: 50,
    smaLongPeriod: 200,
...
ws.on('open', () => {
    console.log(`\n📥 BACKTEST 24H: TP $2.00 | SL $3.00 | M3 | +MACD FILTER...`);
    fetchTicks();
});
...
function runSimulation() {
    ...
    for (let i = 250; i < allTicks.length - LATENCY; i++) {
        ...
        if (!inTrade) {
            ...
            if (sma50 && sma200 && macdOk && (Math.abs(quote - sma50) / sma50 * 100 < CONFIG.distLimit)) {
                if (allUp && quote > sma200) { inTrade = true; tradeType = 'UP'; entryPrice = allTicks[i + LATENCY]; trades++; i += LATENCY; }
                else if (allDown && quote < sma200) { inTrade = true; tradeType = 'DOWN'; entryPrice = allTicks[i + LATENCY]; trades++; i += LATENCY; }
            }
        } else {
    ...
    console.log("\n=========================================");
    console.log("🕵️‍♂️ RESULTADO SNIPER PRO + MACD (TP $2.0)");
    console.log("=========================================");
    console.log(`PnL Neto (24h): $${balance.toFixed(2)} USD 💰`);
