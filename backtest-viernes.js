...
// Viernes pasado (20 de Febrero de 2026)
const startTS = 1771477200; // 2026-02-20 00:00:00 GMT
const endTS = 1771563599;   // 2026-02-20 23:59:59 GMT

console.log(`\n🔍 BACKTEST DETALLADO: ORO - VIERNES 20 FEB`);
console.log(`==========================================================`);
...
ws.on('open', () => {
    ...
    ws.send(JSON.stringify({
        ticks_history: SYMBOL, end: 'latest', count: 500, granularity: 3600, style: 'candles'
    }));
    // Pedir M1
    ws.send(JSON.stringify({
        ticks_history: SYMBOL, end: endTS, start: startTS, granularity: 60, style: 'candles'
    }));
});
...
function runFridayBacktest() {
    ...
    for (let i = 40; i < m1Candles.length; i++) {
        ...
        if (m1S20[i] > m1S40[i] && h1TrendUp) {
            // Retroceso a SMA-40 (PM-40 Setup)
            if (c.low <= m1S40[i] * 1.0002) {
                let resistance = c.high;

                // Buscar ruptura en las siguientes velas
                for (let k = i + 1; k < i + 15 && k < m1Candles.length; k++) {
                    if (m1Candles[k].close > resistance) {
                        ...
                        console.log(`🕒 ${new Date(m1Candles[k].epoch * 1000).toLocaleTimeString()} | ${outcome > 0 ? '✅ PN:' + (pnl.toFixed(2)) : '❌ PN:' + (pnl.toFixed(2))}`);
    ...
    function simulateTrade(startIdx) {
        ...
        for (let j = startIdx + 1; j < m1Candles.length; j++) {
            ...
            const loss = ((m1Candles[j].low - entry) / entry) * MULTIPLIER * STAKE;
    ...
    console.log(`\n--------------------------------------------------`);
    console.log(`🏆 RESUMEN VIERNES 20 FEB:`);
