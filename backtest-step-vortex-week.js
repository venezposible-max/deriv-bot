...
let allTicks = [];
const TOTAL_TICKS_NEEDED = 50000; // Cargaremos 50k ticks (~12-18 horas de alta volatilidad)
...
ws.on('open', () => {
    console.log(`\n📥 EJECUTANDO BACKTEST SEMANAL: STEP INDEX (Vortex 3-5)...`);
    console.log(`🧠 Configuración As-Is: TP: $${SNIPER_CONFIG.takeProfit} | SL: $${SNIPER_CONFIG.stopLoss} | Dist: ${SNIPER_CONFIG.distLimit}`);
    fetchTicks();
});
...
ws.on('message', (data) => {
    ...
    if (msg.msg_type === 'history') {
        ...
        if (allTicks.length < TOTAL_TICKS_NEEDED && chunk.length > 0) {
            ...
            console.log(`\n✅ DATA CARGADA (${allTicks.length} ticks). Iniciando simulación...`);
            runVortexBacktest();
            ws.close();
        }
    }
...
// --- FUNCIONES TÉCNICAS (Copia fiel de server-bot.js) ---
...
function runVortexBacktest() {
    ...
    for (let i = 2000; i < allTicks.length - 100; i++) {
        ...
        if (!inTrade) {
            ...
            if (!trendVortex || !macd || macd.prev === null) continue;

            const distPct = Math.abs(quote - trendVortex) / trendVortex * 100;

            // MOMENTUM CONSECUTIVO (Según momentum: 5)
            ...
            if (isExplosion && distPct < SNIPER_CONFIG.distLimit) {
                if (allUp && quote > trendVortex && macd.current > macd.prev && rsi7 < 80) direction = 'UP';
                if (allDown && quote < trendVortex && macd.current < macd.prev && rsi7 > 20) direction = 'DOWN';
            }
    ...
    console.log("\n=========================================");
    console.log("🕵️‍♂️ RESULTADO SEMANAL: STEP INDEX VORTEX");
    console.log("=========================================");
