...
// Miércoles pasado (18 de Febrero de 2026)
const startTS = 1771304400; // 2026-02-18 00:00:00 GMT
const endTS = 1771390799;   // 2026-02-18 23:59:59 GMT

console.log(`\n🔍 BACKTEST DETALLADO: ORO - MIÉRCOLES 18 FEB`);
console.log(`==========================================================`);
...
ws.on('message', (data) => {
    ...
    if (msg.msg_type === 'candles') {
        if (msg.echo_req.granularity === 3600) {
            h1Candles = msg.candles || [];
            console.log(`✅ H1 Candles: ${h1Candles.length} recibidas.`);
        } else {
            m1Candles = msg.candles || [];
            console.log(`✅ M1 Candles: ${m1Candles.length} recibidas.`);
        }
        if (m1Candles.length > 0 && h1Candles.length > 0) {
            runWednesdayBacktest();
            ws.close();
        }
    } else if (msg.error) {
...
function runWednesdayBacktest() {
    ...
    console.log(`\n--------------------------------------------------`);
    console.log(`🏆 RESUMEN MIÉRCOLES 18 FEB:`);
