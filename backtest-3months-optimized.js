...
let totalPnLGlobal = 0;

console.log(`\n====================================================`);
console.log(`📊 INICIANDO MEGA-BACKTEST DE 3 MESES (DETALLADO)`);
console.log(`====================================================`);
...
function fetchNextWeek() {
    if (currentWeek >= TOTAL_WEEKS) {
        console.log(`----------------------------------------------------`);
        console.log(`TOTAL NETO 3 MESES: $${totalPnLGlobal.toFixed(2)} 💰`);
    ...
    const messageHandler = (data) => {
        ...
        if (msg.msg_type === 'history') {
            ...
            if (weeklyTicks.length < TICKS_PER_WEEK && chunk.length > 0) {
                ...
                console.log(`${(currentWeek + 1).toString().padStart(3)} | ${res.trades.toString().padStart(6)} | ${res.wins.toString().padStart(4)} | ${res.losses.toString().padStart(4)} | $${res.balance.toFixed(2).padStart(9)} | $${totalPnLGlobal.toFixed(2).padStart(9)}`);
                currentWeek++;
                weeklyTicks = []; // Clear memory
                fetchNextWeek();
            }
        }
    };
